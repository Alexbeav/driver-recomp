...
#
# Writes: dist/Driver-Recomp-<VERSION>-<artifact-tag>.zip
set -euo pipefail
...
exec bash "${PACKAGER}" \
  ...
  --zip-prefix Driver-Recomp \
  --exe-name Driver_Recompiled \
  --display-name "Driver Recompiled" \
  --recompiler-build "${RECOMPILER_BUILD}" \
  --version-env RELEASE_VERSION \
  --disc-hint "your legally owned Driver disc" \
